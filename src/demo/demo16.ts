...
import { straightPerspective1, rotateY, rotateX, translate, multiply, inverse } from "./matrix4";
...
export const init = (canvas: HTMLCanvasElement) => {
  ...
  const cameraInitMartix = translate(0, -radius /3, radius * 2)

  watchEffect(() => {
    gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT)

    const cameraMatrix = multiply(rotateY(cameraAngle.value * Math.PI / 180), cameraInitMartix)
