...
let offset = 4
const createDataTexture = (gl: WebGLRenderingContext, data: Float32Array | null, w: number, h: number) => {
...
const createFb = (gl: WebGLRenderingContext, w: number, h: number, data?: Float32Array) => {
...
export const init = (canvas: HTMLCanvasElement) => {
  ...
  const { offset: velocityTex } = createDataTexture(gl, velocity, texWidth, texHeight)
  const fbTexs = [
  ...
  const object = new GLObject({
    uniforms: {
      ...
      u_projectiomMatrix: orthographic(0, canvas.width, 0, canvas.height, -1, 1),
    ...
    attrib: new GLAttrib(
      ...
      { ids: { name: 'a_id', size: 1 }, positions: { size: 2, name: 'a_position'} }
    ),
