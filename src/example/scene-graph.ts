...
import { identity, multiply, rotateX, rotateY, rotateZ, scale, translate } from "./matrix4"


type TRSDataVal = [number, number, number] | [number, number] | [number] | number
type TRSData = {
...
const normalTRSArgs = (x: number, def: number, isRotate = false, y?: number, z?: number): [number, number, number] => {
...
export class SceneNode {
  ...
  getLocalMatrix() {
    const isChange = this.beforeDirtysTRS.value.length > 0 ||
    ...
    if (isChange) {
      ...
      if (beforesTRS.length > 0) {
        matrix = multiply(
          matrix,
          ...beforesTRS.map(trs => trs.getMatrix()),
        )
      }
...
export const getSceneNodeByConfig = <T extends string>(config: SceneNodeConfig<T>) => createSceneNodeByConfig(config)