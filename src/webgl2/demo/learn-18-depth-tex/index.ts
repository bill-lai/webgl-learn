...
import norURI from "./bricks2_normal.jpg";
import diffuseURI from "./bricks2.jpg";
import depthURI from "./bricks2_disp.jpg";
...
const getProps = (gl: WebGL2RenderingContext) => {
  const { tex: depthTex, loaded: depthLoaded } = generateTex(
    gl,
    depthURI,
    gl.CLAMP_TO_EDGE,
    gl.RGBA,
  ...
  return {
    ...
    diffuseTex,
    norTex,
    texLoaded: Promise.all([norLoaded, diffuseLoaded, depthLoaded]),
...
export const init = (canvas: HTMLCanvasElement) => {
  ...
  const redraw = () => {
    ...
    const uniforms = {
      ...
      depthTex: useTex(gl, props.depthTex!, gl.TEXTURE_2D, 2),
      modelMat: modalMat,
      normalMat,
    };
  ...
  return mergeFuns(
    createFPSCamera(
      ...
      (nViewMat, nEysPosition) => {
        mat4.copy(props.uniforms.viewMat, nViewMat);
        vec3.copy(eysPosition, nEysPosition);
        redraw();
      },
      [0, 1, 0],
      [0, 0, 5]
