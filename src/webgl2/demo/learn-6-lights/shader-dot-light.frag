...
void main() {
  ...
  float att = 1.f / (light.constant + light.linear * d + light.quadratic * pow(d, 2.f));

  vec3 oColor = calcFragColor(material, vTexcoord, lightDire, eysDire, nor, light.ambient * att, light.diffuse * att, light.specluar * att);
